...
(async()=>{
 assert.match(read('index.html'),/<script\b[^>]*src=["']\/usage-tracker\.js["'][^>]*data-game=["']haikou["']/);
 for(const f of ['usage-settings.html','usage-settings.js','usage-settings.css'])assert(read(f).length>0);
 const id='日本語テスト',account=digest('haikou:'+id),alias=digest('haikou:'+Buffer.from(id,'utf8').toString('latin1'));
 let reads=0;
 const summary=handler('api/usage-summary.js',{'@vercel/blob':{list:async()=>({blobs:[{pathname:'haikou/u/'+'a'.repeat(64)+'.json'}],hasMore:false}),get:async()=>{reads++;return {statusCode:200,stream:JSON.stringify({id,display:'表示名テスト'})}}}});
 ...
 async function tracker(excludeAtStart,excludeDuring){let excluded=excludeAtStart,calls=[],timers=[];const storage=new Map([['haikou-obake:token','valid']]);const box={document:{currentScript:{dataset:{game:'haikou'}},hidden:false,hasFocus:()=>true},localStorage:{getItem:k=>k==='yg-usage-excluded'?(excluded?'1':null):storage.get(k),setItem:(k,v)=>storage.set(k,v)},crypto:crypto.webcrypto,TextEncoder,Date,performance:{now:()=>0},AbortSignal,setTimeout:f=>timers.push(f),setInterval:()=>{},addEventListener:()=>{},fetch:async(url,options)=>{calls.push({url,options});if(url==='/api/usage-identity'){if(excludeDuring)excluded=true;return {ok:true,json:async()=>({id,displayName:'表示名テスト'})}}return {ok:true}}};vm.runInNewContext(read('usage-tracker.js'),box);for(const f of timers)await f();const sends=calls.filter(c=>c.options?.method==='POST');if(excludeAtStart){assert.equal(calls.length,0)}else if(excludeDuring){assert.equal(sends.length,0)}else{assert.equal(sends.length,1);const b=JSON.parse(sends[0].options.body);assert.equal(b.account,account);assert.equal(b.displayName,'表示名テスト');assert.equal(b.identityKnown,true)}}
