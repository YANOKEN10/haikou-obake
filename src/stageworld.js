...
function buildBranch(scene, opts) {
  ...
  for(const z of [-20,-11,-2,7,14]) for(const sx of [-1,1]) {
    const x=sx*38.18;
    mb.box(x,2.25,z,.12,2.7,1.35,0x718f96);
    mb.box(x,3.68,z,.13,.22,1.65,0xd0c5aa);
    mb.box(x,3.92,z,.13,.22,.95,0xd0c5aa);
    for(const dz of [-.85,.85]) mb.box(x,2.7,z+dz,.18,3.8,.16,0xb9ad95);
  }
  ...
  for(let x=-15.5;x<=15.5;x+=2.3) for(let z=34;z<=61;z+=2.2)
    mb.tuft(x+(z%2)*.18,z,.16,.22,0x244f31,0x4b7d48,2,.12);
  mb.box(0,.18,47.5,.12,.03,27,0xd6d4b1);
  for(const x of [-14,14]) mb.box(x,.18,47.5,.1,.03,27,0xd6d4b1);
  for(const z of [35,60]) {
    mb.box(0,1.05,z,7.2,.14,.14,0xd2d0bd);
    for(const x of [-3.5,3.5]) mb.box(x,.55,z,.14,1.1,.14,0xd2d0bd);
  }
  ...
  // 大食堂の木張り、長机、肖像画、梁。明るい宴会場ではなく廃校らしい沈んだ色にする。
  for(const z of [38,43,48,53,58]) {
    mb.box(47.15,1.55,z,.18,2.75,3.1,0x5b3829);
    mb.box(46.98,2.05,z,.12,1.25,1.1,z%2?0x594e42:0x6c5141);
    mb.box(46.88,2.05,z,.08,1.55,1.4,0xb39a67);
  }
  for(const z of [40,45,50,55]) {
    mb.box(32.5,.78,z,16,.16,1.25,0x714a31);
    for(let x=25;x<=40;x+=2.5) mb.box(x,.45,z+1.0,.38,.9,1.45,0x523629);
  }
  for(let z=38;z<=58;z+=4) mb.box(32.5,4.05,z,28,.2,.28,0x493128);
  // 中庭回廊：石柱とアーチ風の段差、ベンチ、つる草。
  for(const sx of [-1,1]) for(let z=-7;z<=26;z+=5.5) {
    const x=sx*9.6;
    mb.box(x,1.5,z,.5,3,.5,0xaaa18d);
    mb.box(x,3.15,z,.85,.3,.85,0xc2b8a0);
    mb.box(x,3.5,z,.55,.45,.55,0x6c6457);
    if(z%11<1) mb.box(x-sx*.55,.42,z+1.8,1.5,.18,3.0,0x594536);
  }
  for(let i=0;i<34;i++) mb.tuft(rand(-11,11),rand(-10,30),.12,rand(.3,.75),0x25462f,0x486a43,2);
...
function buildPark(scene, opts) {
  ...
  // 曲がりくねった魔法街風の路地。看板、出窓、路面の折れで奥を見通せなくする。
  const alleyPts=[[-15,14],[-8,10],[-13,5],[-6,0],[-11,-5],[-3,-9]];
  for(let i=0;i<alleyPts.length-1;i++){
    const [x1,z1]=alleyPts[i],[x2,z2]=alleyPts[i+1];
    mb.wall(x1-4,z1,x2-4,z2,.1,4.2,.24,i%2?0x46333c:0x343746);
    mb.wall(x1+4,z1,x2+4,z2,.1,4.2,.24,i%2?0x3d4552:0x53363d);
    mb.wall(x1,z1,x2,z2,.08,.14,5.5,i%2?0x54434a:0x424451);
    const mx=(x1+x2)/2,mz=(z1+z2)/2;
    mb.box(mx-4.25,2.2,mz,1.0,1.8,2.1,0x375b68,{rotY:(i%3-1)*.25});
    mb.box(mx+4.25,3.1,mz,.3,2.0,1.4,i%2?0xd35b8b:0x45b9c8);
  }
  addRoom(rooms,spawnSpots,"alley","曲がる夜店の路地","art",-18,-11,2,16,.58);
  ...
  mb.box(-5,1.15,-19,16,2.0,2.1,0x753b42);
  mb.box(4.2,1.45,-19,4.0,2.8,2.8,0x382f39);
  mb.box(6.6,1.1,-19,1.7,1.7,1.7,0xb94d43);
  for(const x of [-11,-7,-3,1,5]) for(const z of [-20,-18])
    mb.box(x,.5,z,.95,.95,.35,0x24262d);
  for(let x=-16;x<=14;x+=5) {
    mb.wall(x,-24,x,-13,.1,6,.16,0x59515d);
    mb.wall(x,-24,x+5,-24,.1,6,.12,0x59515d,{rotY:.2});
  }

  // 気球型ライド。中央塔から色の違う籠を吊り、停止中でも形が読める。
  const bx=-39,bz=43;
  mb.box(bx,5.2,bz,1.2,10.4,1.2,0x794b78);
  mb.box(bx,10.0,bz,5.0,.65,5.0,0x49374d);
  for(let i=0;i<8;i++){
    const a=i/8*Math.PI*2,x=bx+Math.cos(a)*7,z=bz+Math.sin(a)*7;
    mb.wall(bx,bz,x,z,6.7,9.8,.12,0x61576d);
    mb.box(x,7.5,z,2.3,1.7,2.3,i%3===0?0x7b466e:i%3===1?0x376b7c:0x80633e);
    mb.box(x,9.1,z,2.9,2.0,2.9,i%2?0x9b4b73:0x4f7c86);
  }
  addRoom(rooms,spawnSpots,"balloons","止まった空中気球","yard",-48,34,-30,52,.6);

  // 二重ループ。短い発光節で円を描き、既存コースターへ視覚的につなぐ。
  for(const lx of [-16,16]) for(let i=0;i<28;i++){
    const a=i/28*Math.PI*2,x=lx+Math.cos(a)*7,y=7.5+Math.sin(a)*7;
    mb.box(x,y,-34,.45,.45,.7,lx<0?0xc85c4d:0x4aa97b);
    if(i%4===0) mb.box(x,y/2,-34,.18,y,.18,0x47434d);
  }

  // 朽ちた城門。中央アーチを通り抜けると、列車庫とコースター側へ抜ける。
  for(const sx of [-1,1]) {
    mb.box(sx*8,3.3,-9,5.0,6.6,4.2,0x504851);
    mb.box(sx*8,7.1,-9,3.8,1.0,3.4,0x363540);
    mb.box(sx*8,8.1,-9,2.0,1.0,2.0,0x292c34);
  }
  mb.box(0,5.5,-9,11,4.0,3.0,0x56505a);
  mb.box(0,7.8,-9,17,1.0,3.6,0x3c3a43);
  for(const x of [-7,-3.5,0,3.5,7]) mb.box(x,8.8,-9,1.0,1.3,1.0,0x30313a);

  // 水路型ライド：浅い色面、島、壊れた小舟。透明材質を使わず軽量にする。
  mb.slab(18,57,49,64,.12,.07,0x28536b);
  mb.slab(24,53,31,68,.13,.07,0x28536b);
  for(const [x,z] of [[21,60],[28,55],[28,66],[38,60],[46,60]]) {
    mb.box(x,.5,z,3.1,.65,1.5,0x5b5263,{rotY:(x%3-1)*.3});
    mb.box(x,.95,z,1.6,.55,.8,0x3e3d49);
  }
  addRoom(rooms,spawnSpots,"canal","水の止まった舟めぐり","yard",18,53,49,68,.55);
  ...
  // 細い路地は通常の9m格子だけでは角を飛び越すため、曲がり角を道として明示する。
  for(const [x,z] of alleyPts) nav.addNode(x,z,0,"曲がる夜店の路地",0);
